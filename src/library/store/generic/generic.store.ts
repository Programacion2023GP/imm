...
>(
  ...
  const makeStore = (set: any, get: any) => {
    ...
    const baseState: GenericStore<T> & S & { _persist: P } = {
      ...
      setField: (key, value) =>
        set((s: any) => ({
          initialValues: { ...s.initialValues, [key]: value },
        })),
      ...
      fetchData: async (hooks) => {
        ...
        try {
          ...
          if (data.ok) {
            const raw = data.data ?? [];
            const items = hooks?.afterFetch
              ? hooks.afterFetch(raw)
              : (cfg.middlewares.afterResponse?.(raw) ?? raw);
            set({ items });
            log(
              "fetchData",
              "success",
      ...
      postItem: async (item, formData, fetchAfter = true, hooks) => {
        ...
        const payload = hooks?.beforePost
          ? hooks.beforePost(item as T)
          : (cfg.middlewares.beforeRequest?.(item) ?? item);
      ...
      removeItemData: async (item, hooks) => {
        ...
        set({
          items: previous.filter((i: any) => i.id !== (item as any).id),
          loading: true,
        });
        try {
      ...
      request: async (options, callback) => {
        ...
        try {
          ...
          const duration = performance.now() - start;
          if (result.ok) {
            if (options.method !== "GET" && result.message)
    ...
    const extensionState = extension
      ? extension(set as any, get as any, persistAPI)
      : ({} as E);
