    ...
    return safeData.filter((row) => {
      ...
      for (const [field, filterValue] of Object.entries(columnFilters)) {
        ...
        switch (col.filterType) {
          ...
          case "text":
          default:
            if (!rawValue.toLowerCase().includes(filterValue.toLowerCase()))
              return false;
            break;
        }
      }
  ...
  const exportCSV = (dataToExport: T[]) => {
    ...
    const rows = dataToExport
      .map((row) =>
        columns
          .map((col) => {
            const v = String(getNestedValue(row, col.field) ?? "").replace(
              /"/g,
              '""',
            );
            return `"${v}"`;
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        border: `1px solid ${
                          hasFilter ? "rgba(155,34,66,0.25)" : C.border
                        }`,
                        borderRadius: C.r4,
                        padding: "5px 10px",
                        display: "flex",
              ...
              {[
                {
                  icon: <RiFileExcelFill size={14} />,
                  label: "Excel (sel.)",
                  action: () => exportExcel(getExportData(true, sortedData)),
                  color: C.green,
                },
                {
                  icon: <RiFileExcelFill size={14} />,
                  label: "CSV (sel.)",
                  action: () => exportCSV(getExportData(true, sortedData)),
                  color: C.blue,
                },
                {
                  icon: <RiFileTextLine size={14} />,
                  label: "JSON (sel.)",
                  action: () => exportJSON(getExportData(true, sortedData)),
                  color: C.gold,
                },
              ].map((opt) => (
                <ExportBtn key={opt.label} {...opt} C={C} />
        ...
        >
          ...
          >
            ...
            {normalColumns.filter((col) => getColumnStats(col) !== null)
              .length === 0 && (
              <span style={{ fontSize: 12, color: C.text3, padding: "8px 0" }}>
          ...
          >
            {title && (
              <div>
                ...
                {subtitle && (
                  <p
                    style={{ fontSize: 12, color: C.text3, margin: "3px 0 0" }}
                  >
                    {subtitle}
                  </p>
                )}
                ...
                {(
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      cursor: "pointer",
                      background: viewMode === mode ? C.ruby : "transparent",
          ...
          {sortConfig.field && (
            ...
            >
              ...
              <strong style={{ color: C.text2 }}>
                {columns.find((c) => c.field === sortConfig.field)?.headerName}
              </strong>
